...
: Distribution of gestational age and the frequency of preterm birth in the PMS dataset, stratified by albuminuria level. {#tbl-fqr-pmsalbumvsga}
...
![Quantile regression with dithering of `ga`. The curves are quantile regression coefficients for the predictor variables `album`, `female` and `smk`, where the horizontal axis shows the different quantiles that range from zero to one.](figures/fig-fqr-pmsbetasqr.mlr.dithered.svg){#fig-fqr-pmsbetasqr.mlr.dithered width="100%"}
