...
<https://github.com/Lawrence-McCandless/QuantileRegressionEpidemiology>
:::
...
and the results are summarized in Table @tbl-fqr-pmsalbumvsga, which also includes the frequency of preterm births, defined as births shorter than 259 days (37 weeks).
...
: Distribution of gestational age and the frequency of preterm birth in the PMS dataset, stratified by albuminuria level.
...
To examine the influence of albuminuria on gestational age, @fig-fqr-pmsdescriptiveAlbumvsga shows the distribution of gestational age stratified by albuminuria level. The red curve is shifted to the left, which illustrates that infants of mothers with albuminuria have shorter gestational age.

![Distribution of gestational age stratified by albuminuria level. The red curve describes mothers with albuminuria, whereas the black curve represents mothers without albuminuria. The dashed line indicates the threshold for preterm birth ($<259$ days).](figures/fig-fqr-pmsdescriptiveAlbumvsga.svg){#fig-fqr-pmsdescriptiveAlbumvsga width="100%"}
...
From the R output, we have the point estimates $\hat{\beta}_{X [\tau]}=-8.0$ and $\hat{\beta}_{0 [\tau]}=268$, and the equation for the $\tau=0.1$ quantile is given by $Q_{\tau}(Y) = 268.0 - 8.0 X$. By default, the `rq(.)` command uses the `"nid"` option [@davino2014quantile; @koenker2005quantile].
...
: Quantile regression, OLS regression, and logistic regression results. Quantile regression uses `ga` as the dependent variable and `album` as the predictor. The logistic model uses `preterm` as the dependent variable. For quantile regression, the `rq(.)` command uses the default option `se="nid"` to compute standard errors and confidence intervals.
...
Again the regression coefficients are integers. To show this behavior more clearly, Figure @fig-fqr-pmsbetasqr.mlr plots the estimated quantile regression coefficients for the different variables when $\tau$ ranges from zero to one. Random sampling errors are ignored (e.g. 95% confidence intervals). We see that the curves bounce around half-haphazardly because they are stuck on integer values. Because $Y$ and $X$ are discrete, the `quantreg` package will only produce integer valued estimates of the model parameters.
...
Figure @fig-fqr-pmsbetasqr.mlr.dithered shows the results of multivariable quantile regression after averaging over the dithered values of gestational age. Thus Figure @fig-fqr-pmsbetasqr.mlr.dithered is identical to Figure @fig-fqr-pmsbetasqr.mlr, except that the results account for discreteness of gestational age due to rounding. The quantile coefficient curves are much smoother. Thus the results are more plausible because the estimated values of $\hat{\beta}_{1 [\tau]}, \hat{\beta}_{2 [\tau]} \dots , \hat{\beta}_{5 [\tau]}$ are not stuck on the set of integers (i.e. numbers like $-2,-1, 0, 1, 2, \dots$).
